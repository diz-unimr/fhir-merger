...
class FhirPathMatcherTests {
    ...
    private static Stream<Arguments> matchFiltersEncounterAtPeriodStart() {
    ...
    public static Parameters getDummyPatientMerge() {
    ...
    void matchFiltersConditionModule(String recorded, boolean matches) {

        var inputTopic = "bar";
        // pick whole bundle
        var expression =
            "Bundle.where(entry.where(resource.is(Condition) and resource.recordedDate >= @2021-04-15))";
        ...
        bundle
            ...
            .addEntry(
                new BundleEntryComponent().setResource(new Organization()));
    ...
    @ParameterizedTest()
    @CsvSource({"2023-03-08,true", "2017-04-01,false"})
    void matchFiltersProcedureModule(String recorded, boolean matches) {

        var inputTopic = "bar";
        // pick whole bundle
        var expression =
            "Bundle.where(entry.where(resource.is(Procedure) and resource.performed >= @2021-04-15))";
        ...
        var performed = Date.from(LocalDate
            ...
            .toInstant());
        var procedure =
            new Procedure().setPerformed(new DateTimeType(performed));
    ...
    void matchFiltersSingleProcedures(String recorded, boolean matches) {

        var inputTopic = "bar";
        // pick single Procedure
        var expression =
            "Bundle.entry.where(resource.is(Procedure) and resource.performed < @2021-04-15)";
    ...
    @ParameterizedTest
    @MethodSource
    void matchFiltersEncounterAtPeriodStart(String expression,
                                            boolean isMatch) {
        var inputTopic = "test";

        var encounter =
            new Encounter().setPeriod(new Period().setStart(Date.from(LocalDate
                .of(2021, 1, 1)
    ...
    @SuppressWarnings("checkstyle:MagicNumber")
    @Test
    void matchFiltersPatientModule() {

        var inputTopic = "patient";
        var expression =
            "Bundle.entry.where(resource.is(Patient) and resource.meta.lastUpdated < @2022-02-19)";
    ...
    @Test
    void matchFiltersPatientModuleWithPatientPatch() {

        var inputTopic = "patient";
        var expression =
            "Bundle.entry.where(resource.is(Patient) or resource.is(Observation) "
                +
                "or (resource.is(Parameters) and resource.parameter.part.where(name = 'path' and value = 'Patient').exists()))";

        var patientPatch = getDummyPatientMerge();

        var bundle = new Bundle();
        bundle.addEntry(new BundleEntryComponent().setResource(patientPatch));
        bundle.addEntry(
            new BundleEntryComponent().setResource(new Encounter()));
    ...
    @Test
    void matchFiltersPatientModulePatientResource() {
    ...
    void matchFiltersOldEncounterNotP21(String encClass) {

        var inputTopic = "adt";

        final String isNotInpatientEncounterOrWithinDateRange =
            "resource.is(Encounter) and (resource.class.code != 'IMP' or resource.period.start >= @2022-06-14)";
        final String isLocationOrOrganization =
            "resource.is(Location) or resource.is(Organization)";
        ...
        if (encClass.equals("IMP")) {
            assertThat(result.value().getEntry()).as(
                    "we expect encounter resource to be filtered")
            ...
            assertThat(result.value().getEntry()).as(
                ...
                .anySatisfy(a -> assertThat(
                    a.getResource().fhirType()
                        .equals("Organization")).isTrue());
        } else {
            assertThat(result.value().getEntry()).as(
                    "one Encounter is expected")
                .anySatisfy(a -> assertThat(
                    a.getResource().fhirType().equals("Encounter")).isTrue());
            assertThat(result.value().getEntry()).as("one Location is expected")
                .anySatisfy(a -> assertThat(
